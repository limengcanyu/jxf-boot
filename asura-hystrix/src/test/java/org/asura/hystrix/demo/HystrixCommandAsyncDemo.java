package org.asura.hystrix.demo;
...
//    }
    ...
    public void startDemo(final boolean shouldLog) {
        ...
        while (true) {
            ...
            Observable<com.netflix.hystrix.examples.demo.CreditCardAuthorizationResult> o = observeSimulatedUserRequestForOrderConfirmationAndCreditCardPayment();

            final CountDownLatch latch = new CountDownLatch(1);
            o.subscribe(new Subscriber<com.netflix.hystrix.examples.demo.CreditCardAuthorizationResult>() {
                @Override
                ...
                @Override
                public void onNext(com.netflix.hystrix.examples.demo.CreditCardAuthorizationResult creditCardAuthorizationResult) {
                    if (shouldLog) {
    ...
    public Observable<com.netflix.hystrix.examples.demo.CreditCardAuthorizationResult> observeSimulatedUserRequestForOrderConfirmationAndCreditCardPayment() {
        /* fetch user object with http cookies */
        try {
            Observable<UserAccount> user = new com.netflix.hystrix.examples.demo.GetUserAccountCommand(new HttpCookie("mockKey", "mockValueFromHttpRequest")).observe();
            ...
            Observable<PaymentInformation> paymentInformation = user.flatMap(new Func1<UserAccount, Observable<PaymentInformation>>() {
                ...
                public Observable<PaymentInformation> call(UserAccount userAccount) {
                    return new com.netflix.hystrix.examples.demo.GetPaymentInformationCommand(userAccount).observe();
                }
            });

            /* fetch the order we're processing for the user */
            int orderIdFromRequestArgument = 13579;
            final Observable<Order> previouslySavedOrder = new com.netflix.hystrix.examples.demo.GetOrderCommand(orderIdFromRequestArgument).observe();
            ...
            }).flatMap(new Func1<Pair<PaymentInformation, Order>, Observable<com.netflix.hystrix.examples.demo.CreditCardAuthorizationResult>>() {
                @Override
                public Observable<CreditCardAuthorizationResult> call(Pair<PaymentInformation, Order> pair) {
                    return new com.netflix.hystrix.examples.demo.CreditCardCommand(pair.b(), pair.a(), new BigDecimal(123.45)).observe();
                }
            });

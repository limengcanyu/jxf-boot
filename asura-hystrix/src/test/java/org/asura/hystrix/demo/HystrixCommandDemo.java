...
public class HystrixCommandDemo {
    ...
    public void executeSimulatedUserRequestForOrderConfirmationAndCreditCardPayment() throws InterruptedException, ExecutionException {
        ...
        UserAccount user = new com.netflix.hystrix.examples.demo.GetUserAccountCommand(new HttpCookie("mockKey", "mockValueFromHttpRequest")).execute();

        /* fetch the payment information (asynchronously) for the user so the credit card payment can proceed */
        Future<PaymentInformation> paymentInformation = new com.netflix.hystrix.examples.demo.GetPaymentInformationCommand(user).queue();

        /* fetch the order we're processing for the user */
        int orderIdFromRequestArgument = 13579;
        Order previouslySavedOrder = new com.netflix.hystrix.examples.demo.GetOrderCommand(orderIdFromRequestArgument).execute();

        com.netflix.hystrix.examples.demo.CreditCardCommand credit = new com.netflix.hystrix.examples.demo.CreditCardCommand(previouslySavedOrder, paymentInformation.get(), new BigDecimal(123.45));
        credit.execute();
    }

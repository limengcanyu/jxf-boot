package org.asura.mybatis.plus.config;
...
import org.asura.mybatis.plus.entity.DataChangeLog;
import org.asura.mybatis.plus.service.DataChangeLogService;
import org.asura.mybatis.plus.util.UserContext;
import lombok.Data;
...
public class DataChangeRecorderInnerInterceptor implements InnerInterceptor {
    ...
    // 修改处：该服务类使用构造器注入，不能使用@Autowired
    private final DataChangeLogService dataChangeLogService;

    // 修改处：
    public DataChangeRecorderInnerInterceptor(DataChangeLogService dataChangeLogService) {
...
//        // 修改处：将数据处理结果保存到data_change_log表中 非sql
//        DataChangeLog dataChangeLog = new DataChangeLog();
//        dataChangeLog.setOperationResult(JSONObject.toJSONString(operationResult));

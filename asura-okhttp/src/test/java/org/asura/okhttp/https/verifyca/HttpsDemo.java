package org.asura.okhttp.https.verifyca;

import com.fasterxml.jackson.databind.ObjectMapper;
import org.asura.okhttp.model.User;
import org.asura.okhttp.util.OkHttpClientUtil;
import okhttp3.*;
...
public class HttpsDemo {
    ...
    static {
        InputStream ca = HttpsDemo.class.getClassLoader().getResourceAsStream("client/ds.crt");
        // 证书作为value,别名作为key; key只要保证唯一即可，但是最好是生成证书时 起的别名
        String alias = "dengshuai_ca";
        try {
            client = OkHttpClientUtil.getHttpsClient(ca, alias);
        } catch (Exception e) {
            e.printStackTrace();
